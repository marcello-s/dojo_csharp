...
[TestFixture]
public class EndOfLineFilterTests
{
    ...
    [Test]
    public void TextScanner_ScanScript_EndOfLineIsFiltered()
    {
        ...
        using (var fs = new MemoryStream(textScannerInput))
        {
            ...
            var endOfLineFilter = new EndOfLineFilter();
            var textSpans = endOfLineFilter.Apply(spans);
            Assert.That(textSpans, Is.Not.Null);
            Assert.That(textSpans.Any(span => span.SpanType == SpanTypeEnum.EndOfLine), Is.False);
        }
    }
}
