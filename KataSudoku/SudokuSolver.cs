...
namespace KataSudoku
{
    public class SudokuSolver
    {
        ...
        public static int LookForCandidate(
            IEnumerable<int> puzzle,
            int index,
        ...
        )
        {
            ...
            while (
                ...
                || SquareContainsNumber(puzzle, GetSquareFromIndex(index), candidate)
                    && candidate <= 9
            )
            {
                candidate++;
        ...
        public static int GetNextFreeIndex(IEnumerable<int> puzzle, int currentIndex)
        {
            var counter = 1;
            while (
                currentIndex + counter < 9 * 9
                && !puzzle.ElementAt(currentIndex + counter++).Equals(0)
            ) { }
